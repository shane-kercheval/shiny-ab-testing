...
```{r read_simulated_data}
attribution_windows <- as.data.frame(read_csv(file='../shiny-app/simulated_data/attribution_windows.csv'))
prior_data <- as.data.frame(read_csv(file='../shiny-app/simulated_data/website_traffic.csv'))
experiments_all <- as.data.frame(read_csv(file='../shiny-app/simulated_data/experiment_traffic.csv'))
conversion_data <- as.data.frame(read_csv(file='../shiny-app/simulated_data/conversion_rates.csv'))
```
...
experiment_name <- unique(experiments_all$experiment_id)[1]
...
user_conversion_rates <-  experiments_all %>% 
    ...
    #left_join()
    arrange(user_id) %>%
    left_join(attribution_windows, by=c('experiment_id', 'metric_id')) %>%
...
users_to_filter_out <- (user_conversion_rates %>%
    filter(first_joined_experiment >= Sys.Date() - attribution_window))$user_id
...
user_conversion_rates <- user_conversion_rates %>%
    select(-experiment_id, -first_joined_experiment, -attribution_window)
...
max_attribution_window <- max((attribution_windows %>%
...
attribution_windows_current <- attribution_windows %>% filter(experiment_id == experiment_name)
...
temp <- expand.grid(user_id=experiments_current$user_id,
            metric_id=(attribution_windows %>% filter(experiment_id == experiment_name))$metric_id) %>%
