...
user_conversion_rates <- user_conversion_rates %>%
    select(-experiment_id, -first_joined_experiment, -conversion_date, -attribution_windows)
...
#
daily_totals_joined_experiment <- experiments_all %>% 
    filter(experiment_id == experiment_name) %>%
    count(first_joined_experiment)
    

metric_selected <- 'Sign Up'
attribution_window <- (experiments_metrics %>% filter(experiment_id == experiment_name & metric_id == metric_selected))$attribution_windows
inner_join(experiments_all %>% 
               filter(experiment_id == experiment_name),
           conversion_data %>%
               filter(metric_id == metric_selected), by = 'user_id') %>%
    mutate()
    count()
