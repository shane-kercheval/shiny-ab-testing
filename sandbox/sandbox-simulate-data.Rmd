...
create_experiment_visits <- function(start_date, end_date, experiment_paths, experiment_id, variation_names) {
    ...
    experiment_visits <- experiment_visits %>%
        ...
        select(-visit_index) %>%
        rename(first_saw_experiment=visit_date) %>%
    ...
    print(experiment_visits %>%
        count(first_saw_experiment) %>%
        ggplot(aes(x=first_saw_experiment, y=n)) +
        geom_line() +
        labs(title='Number of People Entering Experiment'))
    
    print(experiment_visits %>%
        count(first_saw_experiment, new_user) %>%
        ggplot(aes(x=first_saw_experiment, y=n, color=new_user)) +
        geom_line() +
        labs(title='Number of People Entering Experiment, shows affect of including new returning users in experiment'))
    
    print(experiment_visits %>%
        count(first_saw_experiment, variation) %>%
        ggplot(aes(x=first_saw_experiment, y=n, color=variation)) +
        geom_line())
...
```{r conversion_rates}
simulate_conversion <- function(user_id, historical_conversion_rate) {
    set.seed(user_id)
    return (rbinom(1, 1, historical_conversion_rate))
}
create_conversion_dataset <- function(metric_name, historical_conversion_rate, standard_number_days_to_convert) {
    
    conversion_data <- visits %>% 
        group_by(user_id) %>%
        summarise(first_visit=min(visit_date))

    conversion_data$converted <- as.logical(map_int(conversion_data$user_id,
                                                       ~ simulate_conversion(., historical_conversion_rate)))
    ...
    print(conversion_data %>%
        count(first_visit, converted) %>%
        ggplot(aes(x=first_visit, y=n, color=converted)) +
        geom_line())

    conversion_data <- conversion_data %>%
        filter(converted)

    # NOTE: i had trouble using the same pattern (setting seed) as before to make data reproducable, so for now, it's not
    # setting the seed based on the user_id seemed to changed the distribution
    conversion_offsets <- map_dbl(conversion_data$user_id, ~ rbinom(1, 30, standard_number_days_to_convert/30))
    conversion_data$conversion_date = conversion_data$first_visit + conversion_offsets

    print(conversion_data %>%
        mutate(days_to_convert = as.numeric(conversion_date - first_visit)) %>%
        count(days_to_convert) %>%
        mutate(days_to_convert=factor(days_to_convert)) %>%
        ggplot(aes(x=days_to_convert, y=n)) +
            geom_col())

    conversion_data <- conversion_data %>%
        mutate(metric=metric_name) %>%
        select(user_id, metric, conversion_date)

    stopifnot(nrow(conversion_data) == length(unique(conversion_data$user_id)))
    
    return (conversion_data)
}

metric_name <- metrics_names[1]
historical_conversion_rate <- 0.3
standard_number_days_to_convert <- attribution_windows_days[1]
conversion_data_1 <- create_conversion_dataset(metric_name, historical_conversion_rate, standard_number_days_to_convert)
conversion_data_1
```

```{r conversion_rates_2}
metric_name <- metrics_names[2]
historical_conversion_rate <- 0.2
standard_number_days_to_convert <- attribution_windows_days[2]
conversion_data_2 <- create_conversion_dataset(metric_name, historical_conversion_rate, standard_number_days_to_convert)
conversion_data_2
```

```{r conversion_rates_3}
metric_name <- metrics_names[3]
historical_conversion_rate <- 0.10
standard_number_days_to_convert <- attribution_windows_days[3]
conversion_data_3 <- create_conversion_dataset(metric_name, historical_conversion_rate, standard_number_days_to_convert)
conversion_data_3
```

```{r conversion_rates_4}
# NOTE: in this dataset there is no correlation between talk-to-sales and paying for the product/service

metric_name <- metrics_names[4]
historical_conversion_rate <- 0.08
standard_number_days_to_convert <- attribution_windows_days[4]
conversion_data_4 <- create_conversion_dataset(metric_name, historical_conversion_rate, standard_number_days_to_convert)
conversion_data_4
```

```{r}
conversion_data <- rbind(conversion_data_1,
                         conversion_data_2,
                         conversion_data_3,
                         conversion_data_4)
...
write.csv(experiments_metrics, file='../simulated_data/experiments_metrics.csv', row.names = FALSE)
write.csv(prior_data, file='../simulated_data/prior_data.csv', row.names = FALSE)
write.csv(experiments_all, file='../simulated_data/experiments_data.csv', row.names = FALSE)
write.csv(conversion_data, file='../simulated_data/conversion_data.csv', row.names = FALSE)
```
