...
credible_intervals <- cbind(temp_credible_interval, as.data.frame(do.call("rbind", temp_credible_interval$asdf))) %>%
    select(expired_time_window, metric_id, posterior, estimate, conf.low, conf.high) %>%
    arrange(expired_time_window, metric_id)
...
credible_intervals %>%
    ggplot(aes(x=expired_time_window, y=estimate)) +
        geom_line() +
...
p_values <- function(successes_a, trials_a, successes_b, trials_b) {
    ...
    return (c(p_value = test_results$p.value,
              estimate = as.numeric(test_results$estimate[1] - test_results$estimate[2]),
...
p_value_intervals <- cbind(temp_p_value, as.data.frame(do.call("rbind", temp_p_value$asdf))) %>%
    select(expired_time_window, metric_id, p_value, estimate, conf.low, conf.high) %>%
    arrange(expired_time_window, metric_id)
...
p_value_intervals %>%
    ...
    ggplot(aes(x=expired_time_window, y=estimate)) +
        ...
        geom_ribbon(aes(ymin=conf.low, ymax=conf.high), alpha=0.2) +
        geom_text(aes(label=ifelse(visit_index == 1, percent(estimate), '')), vjust=-1) +
        facet_wrap(~metric_id) +
